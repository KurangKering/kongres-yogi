...
class PendaftaranModel extends Model
{
    ...
    public function getAll()
    {
        ...
        $dt->query('select pend.id as id, pend.tanggal_pendaftaran, pend.nama, pend.tanggal_lahir, pend.institusi, pend.kota, pend.provinsi, pend.no_hp, pend.email, id_event_simposium, pend.status, pend.biaya, pend.kode_unik_pembayaran, simposium.kategori, simposium.hybrid, es.harga, es.tipe_pendaftaran from pendaftaran pend
    ...
    {
        $dt = new Datatables(new Codeigniter4Adapter());
        $dt->query('select validasi.file, validasi.tanggal_validasi, pend.id as id_pendaftaran, pend.tanggal_pendaftaran, pend.nama, pend.tanggal_lahir, pend.institusi, pend.kota, pend.provinsi, pend.no_hp, pend.email, pend.id_event_simposium, pend.status, pend.kode_unik_pembayaran from validasi join pendaftaran pend on pend.id = validasi.id_pendaftaran ');
